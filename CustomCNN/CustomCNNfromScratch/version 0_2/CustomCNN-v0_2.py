...
def calculate_mean_std(
    ...
    Args:
        dataloader (DataLoader): 数据加载器，需要包含未标准化的图像数据

    Returns:
        Tuple[List[float], List[float]]: 返回均值和标准差的列表，每个通道一个值
    """
    # 初始化累加器
    channels_sum: Tensor = torch.zeros(3)  # RGB三通道
    channels_squared_sum: Tensor = torch.zeros(3)
    num_batches: int = 0

    # 遍历数据集
    for data, _ in dataloader:
        # data shape: [batch_size, channels, height, width]
        batch_mean: Tensor = torch.mean(data, dim=[0, 2, 3])  # 计算每个通道的均值
        batch_squared_mean: Tensor = torch.mean(data ** 2, dim=[0, 2, 3])  # 计算每个通道的平方均值

        channels_sum += batch_mean
        channels_squared_sum += batch_squared_mean
        num_batches += 1

    # 计算最终的均值和标准差
    mean: Tensor = channels_sum / num_batches
    std: Tensor = (channels_squared_sum / num_batches - mean ** 2) ** 0.5
...
def get_transforms(
    ...
    Args:
        phase (str): 'train' 或 'val'，指定是训练还是验证阶段
        mean (Optional[List[float]]): 数据集的均值，每个通道一个值
        std (Optional[List[float]]): 数据集的标准差，每个通道一个值
        input_size (Tuple[int, int]): 输入图像的目标尺寸，默认(224, 224)

    Returns:
        transforms.Compose: 图像转换pipeline

    Raises:
        ValueError: 当phase不是'train'或'val'时抛出异常
    """
    ...
    if phase == 'train':
        ...
        train_transforms = [
            ...
            transforms.ColorJitter(
                brightness=0.2,  # 亮度变化范围：±0.2
                contrast=0.2,  # 对比度变化范围：±0.2
                saturation=0.2  # 饱和度变化范围：±0.2
            ),
            # 随机仿射变换
            transforms.RandomAffine(
                degrees=0,  # 不进行旋转
                translate=(0.1, 0.1),  # 平移范围：宽高的±10%
                scale=(0.9, 1.1)  # 缩放范围：0.9-1.1倍
            )
        ]
...
class ImageTransforms:
    ...
    def calculate_mean_std(dataloader: DataLoader) -> Tuple[List[float], List[float]]:
        ...
        Returns:
            Tuple[List[float], List[float]]: 返回均值和标准差的列表，每个通道一个值
        """
        channels_sum: Tensor = torch.zeros(3)
        channels_squared_sum: Tensor = torch.zeros(3)
        num_batches: int = 0

        for data, _ in dataloader:
            batch_mean: Tensor = torch.mean(data, dim=[0, 2, 3])
            batch_squared_mean: Tensor = torch.mean(data ** 2, dim=[0, 2, 3])
    ...
    def __init__(self,
        ...
        Args:
            input_size: 输入图像的目标尺寸
            mean: 数据集的均值，每个通道一个值
            std: 数据集的标准差，每个通道一个值
        """
        self.input_size = input_size
        self.mean = mean or [0.485, 0.456, 0.406]  # 默认使用ImageNet的值
    ...
    def get_train_transforms(self) -> transforms.Compose:
        """
        获取训练数据转换pipeline（包含数据增强）

        Returns:
            transforms.Compose: 训练数据转换pipeline
        """
        return transforms.Compose([
            # 数据增强
            transforms.RandomHorizontalFlip(p=0.5),
    ...
    def get_val_transforms(self) -> transforms.Compose:
        ...
        Returns:
            transforms.Compose: 验证数据转换pipeline
        """
